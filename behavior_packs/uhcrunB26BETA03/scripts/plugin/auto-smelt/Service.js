...
const spawnLoc = (loc) => ({ x: Math.floor(loc.x) + 0.5, y: loc.y + 0.5, z: Math.floor(loc.z) + 0.5 });
...
class Service {
    ...
    flushBatch = (jobs) => {
        ...
        if (jobs.length === 1) {
            ...
            this.spawnLapisRewards(job.player, dimension, { total: lapisTotal, position: lapisPosition });
