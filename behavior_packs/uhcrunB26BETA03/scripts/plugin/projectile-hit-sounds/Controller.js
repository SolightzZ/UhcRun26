...
class Controller {
    onProjectileHitEntity = (ev) => {
        ...
        if (target?.isValid) {
            ...
            shooter.playSound('random.orb', { location: shooter.location, volume: 0.7, pitch: pitch });
            if (target.typeId === 'minecraft:player') {
                shooter.onScreenDisplay.setActionBar(`${target.name} §7(§b${dist.toFixed(1)}m§7)`);
            }
        } else {
            shooter.playSound('random.orb', { location: shooter.location, volume: 0.7, pitch: 0.5 });
        }
    };
}
