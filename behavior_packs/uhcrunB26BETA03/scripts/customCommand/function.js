...
import {
  spawnLeaderboardNPC,
  updateLeaderboard,
} from "../Manager/LeaderboardNPC.js";
...
function setPlayerSpawn(player) {
  ...
  try {
    ...
    console.error(
      "[Command] Failed to set player spawn for",
      player.name,
      ":",
      e,
    );
...
function uhcSetup() {
  ...
  try {
    ...
    // Dynamic ticking areas — load based on default border (500)
    ...
    system.runTimeout(() => {
      try {
        cmd(
          `structure load ${SPAWN_CONFIG.structureName} ${SPAWN_CONFIG.structureLoc}`,
        );
      } catch (e) {
...
// Dynamic ticking areas generator — load only what's needed
