...
class BlockFillerPatternEnqueue {
  ...
  processLayeredTasks() {
    ...
    // จำกัดจำนวนงานที่ใช้งานอยู่ต่อรอบการประมวลผล เพื่อกระจายภาระไปยังหลายรอบการประมวลผล
    // ปรับขนาดตามจำนวนงานที่ใช้งานอยู่ เพื่อไม่ให้เกิดคอขวดที่รูปแบบ 1-2 รูปแบบ
    const MAX_LAYERED_PER_TICK = Math.min(4, Math.max(1, Math.ceil(len / 4)));
    let processed = 0;

    for (let i = len - 1; i >= 0 && processed < MAX_LAYERED_PER_TICK; i--) {
      const lt = this.ACTIVE_LAYERED_TASKS[i];
      if (lt.stopped || !lt.player?.isValid) {
        const lastIdx = this.ACTIVE_LAYERED_TASKS.length - 1;
        if (i !== lastIdx)
          this.ACTIVE_LAYERED_TASKS[i] = this.ACTIVE_LAYERED_TASKS[lastIdx];
      ...
      if (lt.patternTask.rotationCache) {
        this.enqueueRotatedPatternSegments(
          lt.dim,
          y,
          lt.patternTask,
          lt.rotationIndex,
