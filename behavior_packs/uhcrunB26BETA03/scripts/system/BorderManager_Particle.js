...
class BorderManagerParticle {
  ...
  particleRendererGetMolang(width = 8) {
    ...
    // Use cached molang if border color and size haven't changed
  ...
  particleRendererRenderBorderAABB(dim, step, molang) {
    ...
    // กระจายการแสดงผลกลุ่มผู้เล่นไปทั่วทั้งรอบการประมวลผลเพื่อลดภาระอนุภาคต่อรอบการประมวลผล
    ...
    for (let i = offset; i < limit; i++) {
      ...
      const px = loc.x,
        ...
        maxZ = pz + view;
      if (px > east - view)
        this.particleRendererRenderEdge(
          dim,
          east,
          Math.max(north, minZ),
          Math.min(south, maxZ),
  ...
  particleRendererRenderSmall(dim) {
    ...
    const pos = { x: 0, y: BORDER_RENDER.PARTICLE_Y, z: 0 };
    ...
    try {
      ...
      // silently ignore small render failures
    }
  }
  ...
  particleRendererTick(players) {
    ...
    this.particleRendererRenderBorderAABB(
      dim,
      16,
      this.particleRendererGetMolang(8),
    );
  }
}
