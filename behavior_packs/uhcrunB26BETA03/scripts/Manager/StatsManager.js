...
function safeStringify(data, label) {
  try {
    ...
    // ตรวจสอบการรับส่งข้อมูลไป-กลับ — หาก JSON.parse ล้มเหลว แสดงว่าข้อมูลเสียหาย
    JSON.parse(json);
    return json;
  } catch (e) {
    console.error(`[UHC] ${label} data corrupted, skipping save:`, e);
    return null;
  }
}
...
function saveTeamStats() {
  try {
    ...
    const json = safeStringify(data, "teamStats");
    if (json) world.setDynamicProperty("uhc_teamStats", json);
  } catch (e) {
...
function savePlayerStats() {
  try {
    ...
    const json = safeStringify(data, "playerStats");
    if (json) world.setDynamicProperty("uhc_playerStats", json);
  } catch (e) {
...
function handleMultiKill(killer) {
  ...
  world.sendMessage(
    dynamicToast(message, "textures/ui/icons/icon_multiplayer"),
  );
...
function handleFirstBlood(killer, victim) {
  ...
  world.sendMessage(
    dynamicToast(message, "textures/ui/friend_glyph_desaturated"),
  );
  world.sendMessage(message);
...
function resolveDeathCause(victimId) {
  ...
  if (!entry.attackerId)
    return entry.damageType ?? entry.cause ?? "environment";
  return "player";
}
