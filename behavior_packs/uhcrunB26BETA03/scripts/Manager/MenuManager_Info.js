//เมนูแสดงข้อมูลเครดิต, ฟีเจอร์, และ ranks
import { ActionFormData } from '@minecraft/server-ui';
import { openMainMenu } from './MenuManager_Main.js';

//แสดงเครดิตผู้พัฒนาและข้อมูลเกม
export function Credits(player) {
   ...
   form
      ...
      .catch((error) => {
         console.error('[UI_ERROR] Credits form failed:', error);
      });
}

//แสดงรายละเอียดฟีเจอร์ทั้งหมดของ UHC
export function Features(player) {
   ...
   form.divider();
   form.label('             - Sleeplite - ');
   form.button('Back', 'textures/uhc/solightzz');      form
   .show(player)
   .then((res) => {
      if (!res || res.canceled) return;
      openMainMenu(player);
   ...
   .catch((error) => {
      console.error('[UI_ERROR] Features form failed:', error);
   });
}

//แสดง ranks (Coming Soon)
