//ระบบ teleport: สุ่ม, เลือกทีม, เลือกผู้เล่นทั้งหมด
...
import {
   ...
   getOtherUhcPlayers,
   playerTeleport,
   teleportGetAllPlayers,
} from './TeleportManager.js';
...
export function showTeleportForm(player, isAdmin) {
   ...
   const mode = isAdmin
      ? { isAdmin: true, doTeleport: AdminTeleport }
      : { isAdmin: false, doTeleport: playerTeleport };
   ...
   form.show(player).then((res) => {
      ...
      console.error('[TeleportMenu] showTeleportForm error:', error);
   });
}
...
function teleportRandom(player, isAdmin) {
   ...
   if (candidates.length === 0) {
      try {
         ...
         console.error('[TeleportMenu] Failed to send no valid UHC message:', error);
      }
      return;
   }
...
function teleportShowAllPlayers(player, mode) {
   ...
   if (others.length === 0) {
      ...
      form.show(player).then(() => {
         ...
         console.error('[TeleportMenu] teleportShowAllPlayers form error (empty):', error);
      });
      return;
   }
   ...
   form.show(player).then((res) => {
      ...
      console.error('[TeleportMenu] teleportShowAllPlayers form error:', error);
   });
}
...
function teleportShowTeamPlayers(player, teamId, mode) {
   ...
   const teamPlayers = getOtherUhcPlayers(player.id).filter(
      (p) => playerTeamCache.get(p.id) === teamId,
   );
   const targetIds = [];
   const form = new ActionFormData();
   ...
   if (teamPlayers.length === 0) {
      form.body(MENU_MSG.noPlayersAvailable);
      form.button(MENU_MSG.back);
      return form.show(player).then(() => showTeleportForm(player, mode.isAdmin)).catch((error) => {
         console.error('[TeleportMenu] teleportShowTeamPlayers form error (empty):', error);
      });
   }
   ...
   form.show(player).then((res) => {
      ...
      console.error('[TeleportMenu] teleportShowTeamPlayers form error:', error);
   });
}
