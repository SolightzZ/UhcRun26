...
import { teamKillObj, uhcDeathsObj, uhcKillsObj } from './State_Game.js';
...
import { setSpectator } from '../plugin/Util.js';
...
function processVictimDeath(player, victimTeamId, loc) {
   ...
   try {
      ...
      console.error('[DeathManager] Failed to spawn light2 particle:', error);
   }
   ...
   try {
      ...
      console.error('[DeathManager] Failed to spawn light5 particle:', error);
   }
   ...
   system.runTimeout(() => {
      try {
         ...
         try {
            ...
            console.error('[DeathManager] Failed to remove UHC tag:', error);
         }

         try {
            setSpectator(player);
         } catch (error) {
            console.error('[DeathManager] Failed to set game mode to Spectator:', error);
         }
         ...
         enqueueItemVacuum(() => {
            try {
               ...
               try {
                  ...
                  console.error('[DeathManager] Failed to spawn player head:', error);
               }
               ...
               try {
                  ...
                  console.error('[DeathManager] Failed to spawn hopper minecart:', error);
               }
               ...
               for (let ii = 0, iLen = items.length; ii < iLen; ii++) {
                  ...
                  try {
                     ...
                     console.error('[DeathManager] Failed to teleport item to vacuum cart:', error);
                  }
               }
               ...
               system.runTimeout(() => {
                  try {
                     ...
                     console.error('[DeathManager] Failed to remove vacuum cart:', error);
                  }
               }, 30);
            } catch (error) {
               console.error('[DeathManager] Item vacuum execution failed:', error);
            }
         });
      } catch (error) {
         console.error('[DeathManager] Deferred spectator transition failed:', error);
      }
   }, 1);
