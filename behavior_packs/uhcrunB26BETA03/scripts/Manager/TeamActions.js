...
function syncTag(player, oldTeamId, newTeamId) {
  ...
  if (
    typeof oldTeamId === "string" &&
    oldTeamId.length > 0 &&
    player.hasTag(oldTeamId)
  ) {
...
export function joinTeam(player, newTeamId) {
  ...
  if (shouldTrack && !oldTeam && !canJoinTeam(newTeamId)) {
    player.sendMessage(
      dynamicToast(
        TEAM_MENU.serverFull(CONFIG.maxTotalPlayers),
        "textures/ui/cancel",
      ),
    );
...
export function clearAllTeams(executor) {
  ...
  const players =
    allPlayersCache.length > 0 ? allPlayersCache : world.getPlayers();
...
export function clearAllTaguhcAndDynamicProperty(executor) {
  ...
  for (let ti = 0, tLen = TEAMS.length; ti < tLen; ti++) {
    teamStats.set(TEAMS[ti].id, { kills: 0, deaths: 0 });
  }
  ...
  console.warn(
    "[UHC] All tags, dynamic properties, and runtime states cleared.",
  );
}
...
export function openTeamMenu(player) {
  if (
    isGameRunning &&
    player.hasTag("uhc") &&
    !player.hasTag(CONFIG.adminTag)
  ) {
    player.sendMessage(
      dynamicToast(TEAM_MENU.cannotChangeMidGame, "textures/ui/cancel"),
    );
    player.playSound("note.bassattack");
    return;
  }
  ...
  form.show(player).then((res) => {
    ...
    if (selection < teamsLen) {
      ...
      if (currentTeamId === selectedTeam.id) {
        player.playSound("note.bassattack");
        player.sendMessage(
          dynamicToast(TEAM_MENU.alreadyOnTeam, selectedTeam.icon),
        );
      ...
      if (!currentTeamId && !canJoinTeam(selectedTeam.id)) {
        ...
        player.sendMessage(
          dynamicToast(
            TEAM_MENU.serverFullShort(CONFIG.maxTotalPlayers),
            "textures/ui/cancel",
          ),
        );
        system.run(() => openTeamMenu(player));
      ...
      player.sendMessage(
        dynamicToast(
          `Joined ${selectedTeam.color}${selectedTeam.name}`,
          selectedTeam.icon,
        ),
      );
      system.run(() => openTeamMenu(player));
    ...
    switch (actionIndex) {
      case 0: {
        if (!currentTeamId || !currentTeam) {
          player.playSound("note.bassattack");
          player.sendMessage(
            dynamicToast(TEAM_MENU.noTeam, "textures/ui/cancel"),
          );
          system.run(() => openTeamMenu(player));
          return;
        }
