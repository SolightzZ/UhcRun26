//เมนู admin สำหรับจัดการทีม, stats, teleport, cache
...
function showDumpViewer(admin, title, body, logTag) {
   ...
   form.body(body);      form.button(MENU_MSG.console, 'textures/ui/icons/icon_fall');
   ...
   form.show(admin).then((res) => {
      ...
      console.error('[AdminMenu] showDumpViewer form error:', error);
   });
}
...
function viewAllMaps(admin) {
   ...
   const dumpMap = (label, map, formatter) => {
      ...
      // ทำซ้ำโดยตรงโดยไม่ใช้ Array.from() เพื่อหลีกเลี่ยงการคัดลอกแผนที่ขนาดใหญ่
      ...
      for (const entry of iter) {
         ...
         try {
            ...
            console.error('View All Maps: ' + error);
            body += ' §c<format error>\n';
         }
         count++;
         if (count > 200) {
   ...
   dumpMap(
      ...
      deathLocation,
      (k, v) =>
         ` §7${resolveName(k)} §8: §c${v.x.toFixed(0)}, ${v.y.toFixed(0)}, ${v.z.toFixed(0)}\n`,
   );
   dumpMap(
      'multiKill',
      multiKill,
      (k, v) => ` §7${resolveName(k)} §8: §cCount:${v.count} Tick:${v.tick}\n`,
   );
   ...
   dumpMap(
      ...
      hitRegistry,
      (k, v) =>
         ` §7Victim:${resolveName(k)} §8<- §cAttacker:${resolveName(v.attackerId)} §8(Tick:${v.tick})\n`,
   );
...
//แสดงรายชื่อผู้เล่นทั้งหมดให้ admin เลือกจัดการทีม
function Managements(admin) {
   ...
   if (pLen === 0) {
      ...
      form.button(MENU_MSG.back);
      return form.show(admin).then(() => AdminMenu(admin)).catch((error) => {
         console.error('[AdminMenu] Managements form error (empty):', error);
      });
   }
   ...
   form.show(admin).then((res) => {
      ...
      console.error('[AdminMenu] Managements form error:', error);
   });
}

//GUI ให้ admin เลือกทีมให้ผู้เล่น
function editPlayerMenu(admin, target) {
   ...
   form.show(admin).then((res) => {
      ...
      if (res.selection <= TEAMS.length) {
         ...
         admin.sendMessage(
            `[/] §aMoved §f${target.name} §ato ${selectedTeam.color}${selectedTeam.name}§a.`,
         );
         return Managements(admin);
      }
      ...
      console.error('[AdminMenu] editPlayerMenu form error:', error);
   });
}
...
function clearTeams(player) {
   ...
   form.show(player).then((res) => {
      ...
      console.error('[AdminMenu] clearTeams form error:', error);
   });
}

//แสดงรายชื่อผู้เล่นทั้งหมดเรียงตามทีม
function playerLists(player) {
   ...
   const players = getCachedPlayers()
      ...
      .sort((a, b) => {
         ...
         const indexA =
            typeof teamA === 'string'
               ? (TEAM_INDEX_MAP.get(teamA) ?? Number.MAX_SAFE_INTEGER)
               : Number.MAX_SAFE_INTEGER;
         const indexB =
            typeof teamB === 'string'
               ? (TEAM_INDEX_MAP.get(teamB) ?? Number.MAX_SAFE_INTEGER)
               : Number.MAX_SAFE_INTEGER;
   ...
   form.show(player).then((res) => {
      ...
      console.error('[AdminMenu] playerLists form error:', error);
   });
}

//แสดงประวัติ kill/death จาก scoreboard objective
function killList(player) {
   ...
   if (history === '') {
      ...
      form.button('Console');
      form.button('Back', 'textures/ui/arrow_left_white');   form.show(player).then((res) => {
      if (!res) return;
      if (res.canceled) return;
      if (res.selection === 0) {
      ...
      console.error('[AdminMenu] killList form error (empty history):', error);
   });
      return;
   }
   ...
   form.show(player).then((res) => {
      ...
      console.error('[AdminMenu] killList form error:', error);
   });
}
...
export function AdminMenu(player) {
   ...
   form.show(player).then((res) => {
      ...
      console.error('[AdminMenu] AdminMenu form error:', error);
   });
}
